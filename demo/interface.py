...
Upload a PDF or TXT file above. We’ll extract the text and compute a match score.

### 2. Retrieval modes
- **Generic RAG**: one-pass FAISS lookup on the full JD.  
- **Fusion RAG**: splits the JD into sub‑queries, retrieves each, then fuses their scores.

### 3. Model choices
Pick a small model (`distilgpt2`) for speed or larger ones (`gpt2-medium`) for quality.

### 4. Data privacy
Everything runs **locally**—no API keys or external calls.
"""
...
st.title("📄 SmartCandidate Analyzer (Simple RAG)")
